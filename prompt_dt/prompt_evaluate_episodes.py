...
def prompt_evaluate_episode_rtg(
    ...
    state = env.reset()
    action_mask_dim=env.action_space.shape[0]

    if mode == 'noise':
        state = state + np.random.normal(0, 0.1, size=state.shape)
    state_padding_length = state_dim - state.shape[0]
    padded_state = np.pad(state, (0, state_padding_length), 'constant', constant_values=(0, 0))

    # we keep all the histories on the device
    # note that the latest action and reward will be "padding"
    states = torch.from_numpy(padded_state).reshape(1, state_dim).to(device=device, dtype=torch.float32)
    actions = torch.zeros((0, act_dim), device=device, dtype=torch.float32)
    rewards = torch.zeros(0, device=device, dtype=torch.float32)
    ...
    for t in range(max_ep_len):
        ...
        actions[-1] = action
        action = action.detach().cpu().numpy()
        action=action[:action_mask_dim]

        state, reward, done, infos = env.step(action)
        state = np.pad(state, (0, state_padding_length), 'constant', constant_values=(0, 0))
