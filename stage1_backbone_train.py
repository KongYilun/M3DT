...
def experiment_mix_env(
    ...
    seed = variant['seed']
    cur_dir = os.getcwd()
    save_path = os.path.join(cur_dir, f'model_saved_backbone/')
    ...
    n_parameters=report_parameters(model)
    model = model.to(device=device)
    task_config = os.path.join('config', f"SuperLong/task_160_seed{seed}.json")
    print(f"SuperLong/task_160_seed{seed}.json")
    ...
    if '160' in variant['prefix_name']:
        print('training on 160 tasks!')
        train_env_name_list, test_env_name_list = task_config.mt160_task_list, task_config.mt160_task_list
        print(len(train_env_name_list))
    elif '120' in variant['prefix_name']:
        print('training on 120 tasks!')
        train_env_name_list, test_env_name_list = task_config.mt120_task_list, task_config.mt120_task_list
        print(len(train_env_name_list))
    elif '80' in variant['prefix_name']:
        print('training on 80 tasks!')
        ...
        raise NotImplementedError("please use the 160, 80, 10")
    ...
    print(timestr)
    exp_prefix = f"{variant['prefix_name']}-{n_parameters}-seed{seed}-{timestr}"
    if log_to_wandb:
        wandb.init(
            name=exp_prefix,
            group=group_name,
    ...
    for iter in range(variant['max_iters']):
        ...
        # if (iter+1) % 200000 == 0:
        #     trainer.save_model(
        #         env_name=args.prefix_name, 
        #         postfix='_iter_'+str(iter+1), 
        #         folder=save_path)
    ...
    trainer.save_model(env_name=args.prefix_name,  postfix='_iter_'+str(iter+1),  folder=save_path, seed=seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prefix_name', type=str, default='mt160')
