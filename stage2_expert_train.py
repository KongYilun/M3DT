...
from prompt_dt.prompt_decision_transformer_with_moe import PromptDTExpert
...
def experiment_mix_env(
    ...
    seed = variant['seed']
    cur_dir = os.getcwd()
    save_path = os.path.join(cur_dir, f'model_saved_expert/')
    ...
    model_with_expert=PromptDTExpert(
    ...
    report_parameters(model_with_expert)
    model_with_expert = model_with_expert.to(device=device)
    # for name, param in model_with_expert.named_parameters():
    #     if param.requires_grad==True:
    #         print(name)
    #############################
    expert_id=variant['expert_id']
    expert_num=variant['expert_num']
    #############################gradient/gradient_grouped_task_{group_num}_{model_scale}M_seed{seed}.json'
    task_config = os.path.join('config', f"SuperLong/moe/gradient/gradient_grouped_task_{expert_num}_seed{seed}.json")
    with open(task_config, 'r') as f:
        task_config = json.load(f)
    train_env_name_list, test_env_name_list = task_config[f'expert_{expert_id}'], task_config[f'expert_{expert_id}']
    print(train_env_name_list)
    # train_env_name_list, test_env_name_list = ['push-v2'], ['push-v2']
    info = get_env_list_for_long_task(train_env_name_list, device, seed=seed,total_env='train')

    exp_prefix = f"{variant['prefix_name']}_seed{seed}"#######-{timestr}
    group_name = variant['prefix_name']
    save_path += exp_prefix
    os.makedirs(save_path, exist_ok=True)
    # for i in range(len(train_env_name_list)):
    #     print(train_env_name_list[i],env_list[i].action_space.shape)
    ...
    #             project='slt',
    #             config=variant,
    #             dir='/home/kyl/code/mtrl/slt-dt/wandb/new'
    #         )
    ...
    for iter in range(variant['max_iters']):#variant['max_iters']variant['max_iters']
        ...
        if (iter+1) % variant['save_interval'] == 0:
            model_with_expert.save_expert(expert_id=expert_id, postfix='_iter_'+str(iter+1),folder=save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-interval', type=int, default=200000)
    ...
    parser.add_argument('--data_path', type=str, default='/amax/datasets/kyl_data/mt160')
